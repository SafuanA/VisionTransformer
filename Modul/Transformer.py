...
class Transformer(nn.Module):
    def __init__(self, embed_dim=512, n_blocks = 6, n_heads=4, ff_dim=2048, dropout=0.1, n_mels=80, output_dim=512, input_dim=312):
        super().__init__()
        self.encoder = TransformerEncoder(embed_dim, n_blocks, n_heads, ff_dim, dropout, n_mels, output_dim, use_linear_in=True)
        #input dim needed for mask of the attention logits
        self.decoder = TransformerDecoder(embed_dim, n_blocks, n_heads, ff_dim, dropout, n_mels, output_dim, input_dim, use_linear_in=True, use_linear_out=True)
...
class TransformerEncoder(nn.Module):
    def __init__(self, embed_dim=512, n_blocks = 6, n_heads=4, ff_dim=2048, dropout=0.1, n_mels=80, output_dim=512, use_linear_in=False, use_linear_out=False):
        super().__init__()

        norm = nn.LayerNorm(embed_dim, eps=1e-12)
        self.use_linear_in = use_linear_in
...
class TransformerDecoder(nn.Module):
    def __init__(self, embed_dim=512, n_blocks = 6, n_heads=4, ff_dim=2048, dropout=0.1, n_mels=80, output_dim=512, input_dim=312, use_linear_in=False, use_linear_out=False):
        super().__init__()
        norm = nn.LayerNorm(embed_dim, eps=1e-12)
        self.use_linear_in = use_linear_in
        self.use_linear_out = use_linear_out
    ...
    def forward(self,x, memory):
        ...
        for layer in self.decoders:
            x = layer(x, memory, True)

        if self.use_linear_out:
            x = self.linear_out(x)
        return x

class BaseEncoderDecoderLayer(nn.Module):
...
class TransformerEncoderLayer(BaseEncoderDecoderLayer):
...
class TransformerDecoderLayer(BaseEncoderDecoderLayer):
    def __init__(self, input_dim, embed_dim, n_heads=4, ff_dim=2048, dropout=0.1, norm=None):
        ...
        self.ff = PositionWiseFeedForward(embed_dim, ff_dim, dropout)
        self.mask = self.make_attention_mask(input_dim)

    def make_attention_mask(self, input_dim):
        mask = torch.ones(1)
        mask = mask.repeat(input_dim, input_dim)
