...
class WavAugment(object):
    #ToDo build Noises from voxceleb train or other form of noises
    def __init__(self, noise_csv_path="train.csv", rir_csv_path="rir.csv"):
        self.noise_paths = pd.read_csv(noise_csv_path)["wav"].values
        self.rir_paths = pd.read_csv(rir_csv_path)["noise_path"].values
        print('augmentation initialised with '+noise_csv_path+' and '+rir_csv_path)
    ...
    def reverberate(self, waveform):
        ...
        if len(rir.shape) > 1:
            return waveform # no augmentation for now
...
if __name__ == "__main__":
    aug = WavAugment('wincsv/train.csv', 'wincsv/rir.csv')
    waveform = load_audio('input2.wav')
