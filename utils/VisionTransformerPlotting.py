...
def CreateVisionTransformer(pretrain, n_classes, shuffle_type, seconds, FcLayer = False):
    ...
    return VisionTransformer(img_size=(80, timeDim), patch_size=(80,1), in_chans=1,
                            ...
                            pretrain=pretrain, norm_file='mean_std_80.npy', n_classes=n_classes,
...
class VisionTransformer(nn.Module):
    def __init__(self, img_size=(80, 104), patch_size=(80,1), in_chans=1,
                 ...
                 norm_file='mean_std.npy', pretrain=False, n_classes=1211, 
...
class Attention(nn.Module):
    ...
    def forward(self, x):
        B, N, C = x.shape
        qkv = self.qkv(x).reshape(B, N, 3, self.num_heads, self.head_dim).permute(2, 0, 3, 1, 4) #B, N, 3 , nH, dH -> 3, B,  dH, N, dH
        q, k, v = qkv.unbind(0) #B,  dH, N, dH
        ...
        if self.fused_attn:
            ...
            #)
            #according to docu same as calling F.scaled_dot_product_attention https://pytorch.org/docs/stable/generated/torch.nn.functional.scaled_dot_product_attention.html
            #attn_mask = torch.ones(L, S, dtype=torch.bool).tril(diagonal=0) if is_causal else attn_mask
            #attn_mask = attn_mask.masked_fill(not attn_mask, -float('inf')) if attn_mask.dtype==torch.bool else attn_mask
            attn_weight = torch.softmax((q @ k.transpose(-2, -1) / math.sqrt(q.size(-1))), dim=-1)
            attn_weight = torch.dropout(attn_weight, self.attn_drop.p, False)
            self.attnLgtCache = attn_weight
            x = attn_weight @ v
            self.attnCache = x
        else:
            q = q * self.scale
